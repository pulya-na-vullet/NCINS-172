...
Ключевые отличия от наивного CQL по displayName:
1) Сначала резолвим пользователя в username / userKey
2) Ищем через CQL: creator = "username" (и fallback contributor)
3) Корректно разбираем ответ /rest/api/search (поля лежат в result.content)
4) Есть fallback: обход пространств + фильтрация history.createdBy

Учётные данные — только из окружения / CLI, не хардкодить PAT в файл.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--url",
        default=os.getenv("CONFLUENCE_URL", "https://confluence.moscow.alfaintra.net"),
        help="Базовый URL Confluence",
    )
    p.add_argument(
        "--username",
        default=os.getenv("CONFLUENCE_USERNAME", ""),
        help="Логин (для Server/DC + PAT почти всегда нужен)",
    )
    p.add_argument(
        "--token",
        default=os.getenv("CONFLUENCE_TOKEN") or os.getenv("API_TOKEN", ""),
        help="PAT / пароль (или env CONFLUENCE_TOKEN)",
    )
    p.add_argument(
        "--user",
        action="append",
        dest="user_hints",
        default=None,
        help="Вариант имени/логина/email автора (можно несколько раз)",
    )
    p.add_argument(
        "--output",
        default=os.getenv("OUTPUT_DIR", "./confluence_pdfs_Zabaryanskiy"),
    )
    p.add_argument(
        "--cache",
        default=os.getenv("CACHE_FILE", "./confluence_pages_cache_Zabaryanskiy.json"),
    )
    p.add_argument("--workers", type=int, default=int(os.getenv("DOWNLOAD_WORKERS", "5")))
    p.add_argument(
        "--wkhtmltopdf",
        default=os.getenv(
            "WKHTMLTOPDF_PATH",
    ...
    p.add_argument(
        "--ssl-verify",
        action="store_true",
        help="Включить проверку SSL (по умолчанию выключена для корп. Confluence)",
    )
    p.add_argument(
        "--force-rescan",
        action="store_true",
        help="Игнорировать кеш и искать заново",
    )
    p.add_argument(
        "--fallback-scan",
        action="store_true",
        help="Если CQL пустой — обойти пространства и фильтровать creator",
    )
    p.add_argument(
        "--spaces",
        default="",
        help="Список space key через запятую (для fallback или ограничения)",
    )
    p.add_argument(
        "--max-spaces",
        type=int,
        default=0,
        help="Ограничить число пространств в fallback (0 = все)",
    )
    p.add_argument(
        "--scan-workers",
        type=int,
        default=4,
        help="Потоки для fallback-обхода пространств",
    )
    p.add_argument(
        "--list-only",
        action="store_true",
        help="Только найти и сохранить список страниц, без PDF",
    )
    p.add_argument(
        "--no-native-pdf",
        action="store_true",
        help="Не использовать /spaces/flyingpdf/..., только html→pdfkit",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    if pages is None:
        ...
        if not identity:
            safe_print(
                "\n💡 Подсказка: откройте профиль пользователя в Confluence и возьмите "
                "точный Username (логин), затем запустите:\n"
                "  python export_user_pages.py --user <USERNAME> --token ...\n"
            )
            return 1
        ...
        if (not pages and args.fallback_scan) or (args.fallback_scan and space_keys):
            if not pages:
        ...
        if pages:
            ...
            safe_print("\n❌ Страницы не найдены.")
            safe_print("Попробуйте:")
            safe_print("  1) --user <точный_логин_из_профиля>")
            safe_print("  2) --fallback-scan  (медленно, но надёжно)")
            safe_print("  3) --fallback-scan --spaces KEY1,KEY2")
            return 1
